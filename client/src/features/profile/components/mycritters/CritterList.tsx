...
export const CritterList = ({ roster, selectedCritter, onCritterSelect }: CritterListProps) => {
  ...
  const mockCritterLevels = {
    "Aqualing": { level: 18, star: "83%" },
    "Cogling": { level: 16, star: "74%" },
    "Sylvan Sentinel": { level: 14, star: "65%" },
    "Strikon": { level: 12, star: "50%" }
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 h-full">
      ...
      <div className="space-y-3">
        {roster.map((critter) => {
          const details = mockCritterLevels[critter.name as keyof typeof mockCritterLevels] || { level: 1, star: 'N/A' };
          return (
            <div 
              key={critter.id} 
            ...
            >
              ...
              <div className="flex-grow">
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  ...
                  <span className="bg-gray-200 px-2 py-0.5 rounded-full text-xs font-semibold">Level {details.level}</span>
                </div>
              </div>
